...
import com.cashir.jpa.specification.SearchCriteria.Type
...
class SpecificationBuilderTest {
    ...
    fun shouldBuildAndCriteria() {
        ...
        criteria.forEach {
            verify {
                spec.and(EntitySpecification<Any>(it, PredicateBuilder(typeConverter)))
            }
        }
    ...
    fun shouldBuildSecurityCriteria() {
        val criteria = listOf(mockCriteria(Type.SECURITY), mockCriteria(Type.SECURITY))
        val spec = mockSpecificationChain()

        val result = specificationBuilder.build<Any>(criteria)
        criteria.forEach {
            verify {
                spec.or(EntitySpecification<Any>(it, PredicateBuilder(typeConverter)))
            }
        }
        assertEquals(spec, result)
    }
